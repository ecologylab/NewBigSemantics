...
function downloadersInfoFactory(): Middleware {
  return (req, res, next) => {
    let workersUrl = `http://${serviceOptions.dpool_service.host}:${serviceOptions.dpool_service.port}/workers.json`;
...
export function create(options: BSPhantomOptions): Promise<MiddlewareSet> {
  ...
  return bs.onReadyP().then(() => {
    ...
    if (serviceOptions.metadata_cache.mongo) {
      cache = new MongoMetadataCache({
        repoMan: bs.getRepoMan(),
        mongo: {
          url: serviceOptions.metadata_cache.mongo.url,
          collection: serviceOptions.metadata_cache.mongo.collection,
        },
      });
    } else {
