  ...
  return (req, res, next) => {
    ...
    mc.getObject("cache", hashedUrl, (err, stream) => {
      ...
      if (!err) {
        ...
        let reqUrl = `http://${dpoolCacheOptions.dpoolService.host}:${dpoolCacheOptions.dpoolService.port}/proxy?url=` + encodeURIComponent(url);
...
export function create(): Promise<MiddlewareSet> {
  return new Promise<MiddlewareSet>((resolve, reject) => {
    let mc = new Minio({
      ...
      accessKey: dpoolCacheOptions.minio.accessKey,
      secretKey: dpoolCacheOptions.minio.secretKey,
